...
for i, start_idx in enumerate(start_indices):
    ...
    for i, model_name in enumerate(model_list):
        ...
        n_hist = 120
        hist_start = len(base_series) - n_hist
        # plt.plot(df['date'][hist_start:], base_series[hist_start:], label='Historical')
        plt.plot(base_series[hist_start:], label='Historical')
        ...
        forecast_dates = pd.date_range(df['date'][hist_start:].iloc[-1], periods=n_forecast + 1, freq='ME')[1:]

        forecast = forecasts[model_name]
        # plt.plot(forecast_dates,forecast, label=f'forcast _ {model_name}')
        plt.plot(forecast, label=f'forcast _ {model_name}')
        plt.ylabel('SPI')
        plt.legend(fontsize=8)
    fig19.suptitle(f'forcast spi_{num}')
plt.tight_layout()
plt.show()
# fig.savefig("spi_plots.png", dpi=300, bbox_inches="tight")
# fig_wt.savefig("spi_wavelet_plots.png", dpi=300, bbox_inches="tight")
# fig_acf_pacf.savefig("spi_acf_pacf_plots.png", dpi=300, bbox_inches="tight")
