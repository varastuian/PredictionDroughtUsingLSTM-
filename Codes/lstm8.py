...
for i, model_name in enumerate(plot_order):
    if model_name in ['WBRF','WBi-LSTM']:
        ...
        true_vals = y_test
    plt.subplot(5,2,i+1)
    plt.plot(true_vals, label='actual spi', marker='o', markersize=3)
...
def forecast_non_wavelet(model_name):
    ...
    current_input = processed_series[-max(selected_lags):].tolist()  # length = max(selected_lags)
    forecast = []
    for i in range(n_forecast):
...
for i, model_name in enumerate(model_list):
    ...
    plt.plot(range(hist_start, len(base_series)), base_series[hist_start:], label='Historical', marker='o', markersize=3)
    ...
    plt.plot(range(len(base_series), len(base_series) + len(test_pred)), test_pred, label='Test Pred', marker='x', linestyle='--')
    ...
    plt.plot(range(len(base_series) + len(test_pred), len(base_series) + len(test_pred) + n_forecast),
             forecast, label='Forecast', marker='o', linestyle='--', color='red')
    plt.title(model_name, fontsize=10)
    plt.xlabel('Time Index (months)')
    plt.ylabel('SPI')
    plt.legend(fontsize=8)
