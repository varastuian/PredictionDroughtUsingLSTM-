...
def set_cell_width(cell, width_twips):
    """Set fixed column width in twips (1 inch = 1440 twips)."""
    tc = cell._tc
...
def add_full_row_top_border(row):
    """Add a horizontal line across the full row."""
...
# Column widths (twips). Corr = 864 (0.6 inch)
...
for station, station_group in df.groupby("station"):
    ...
    for spi, spi_group in station_group.groupby("spi", sort=False):
        ...
        for idx, (_, r) in enumerate(spi_group.iterrows()):
            ...
            # only first row of timescale shows label
            if idx == 0:
                row[1].text = str(spi)
    ...
    # add horizontal border for full station block
